//DAN_RPG_2.3 = introduzione di piu' commenti e finito la risposta C dell'interazione con il maranza
...
//per adesso il comando class non serve a niente
...
string pname; //INIZIO DEL GIOCO e chiedo all'utente il nome da inserire
...
//qui il giocatore sceglie la classe
...
//questa e' una variabile che prende numeri a caso, per ora non serve
srand(time(NULL));
...
if (risposta_bianco == "bianco"|| risposta_bianco == "Bianco" || risposta_bianco == "BIANCO")
{
    ...
    cout<<""<<endl; //qui il giocatore guadagna soldi e xp
    int money = 5;
    float xp = 1;
...
break;
    ...
    {
        string scappa = "*L'avventuriero scappa dal maranza con il mago*";

        for (char maronzo : maranza )
        {
            cout<<maronzo<<flush;
            this_thread::sleep_for(chrono::milliseconds(100));
        ...
        for (char scappo : scappa)
            ...
            string fr = "UN GIORNO TI PICCHIERO'!!!";
      ...
      else if (rm != "A" && rm != "a" && rm != "b" && rm != "B" && rm != "c" && rm != "C")
    {
        cout<<"Errore, ripeti."<<endl;
    }
}

for (char pausan : pausa)
{
    cout<<pausan<<flush;
    this_thread::sleep_for(chrono::milliseconds(500));
}

//dopo l'incontro col maranza il mago porta l'avventuriero al bar
